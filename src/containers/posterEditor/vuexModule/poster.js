...
import { Message } from "element-ui";
let posterItems = sessionStorage.getItem('posterItems')
posterItems ? posterItems = JSON.parse(posterItems) : posterItems = []  
function getState() {
  const state = {
    activityId: "",
    pageConfigId: "",
...
const mutations = {
  ...
  // 添加网格布局
  [MTS.ADD_LAYOUT_WIDGET](state, items) {
    // state.assistWidgets = items;
  },
  // 设置图层面板的打开关闭状态
...
const actions = {
  ...
  addItem({ commit, dispatch, state }, item) {
    ...
    if (widgetCountLimit) {
      const currentCount = state.posterItems.filter(i => i.type === item.type)
        .length;
      if (currentCount >= widgetCountLimit) {
        Message.error(
          `<${item.typeLabel ||
            item.type}>类型的组件最多有${widgetCountLimit}个`
        );
        return;
      }
    }
  ...
  addLayoutWidget({commit}, item)  {
    commit(MTS.ADD_LAYOUT_WIDGET, item)
  },
  ...
  updateWidgetState(
    ...
    if (widgetItem) {
      ...
      if(index != -1) {
        keyPath = keyPath.split('.')[1]
        _set(widgetItem.wState.style[index], keyPath, value) 
      }else {
